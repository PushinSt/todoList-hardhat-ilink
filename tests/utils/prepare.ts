...
export async function prepareERC20Tokens(thisObject: Mocha.Context, signer: SignerWithAddress) {
    ...
    const tokenFactory2 = await ethers.getContractFactory("TodoList")
    const token4 = await tokenFactory2.connect(signer).deploy()
    await token4.deployed()
    thisObject.token4 = token4
}


export async function prepareTodoListTokens(thisObject: Mocha.Context, signer: SignerWithAddress) {
    const tokenFactory = await ethers.getContractFactory("TodoList")
